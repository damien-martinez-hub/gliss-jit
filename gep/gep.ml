...
let make_env info =
	...
	let endianness _ =
		match Irg.get_string_let "endianness" with
		| None ->
	...
	("HOST_ENDIANNESS", Templater.TEXT (fun out -> output_string out (if Sys.big_endian then "big" else "little"))) ::
	("TARGET_ENDIANNESS", Templater.TEXT (fun out -> output_string out (endianness ()))) ::
			...
			begin
				...
				if not !no_default then begin
					...
					if not !gen_with_trace then begin
						...
						(* !!TODO!! output the correct decoder, not only decode_dtrace
				...
				(* generate config.h *)
				let config_path = "include/" ^ info.Toc.proc ^ "/config.h" in
				let (do_tpl, tpl) =
					if !config_tpl <> "" then (true, !config_tpl) else
					if Sys.file_exists config_path then (false, "") else
					(true, "") in
				if do_tpl then
					if not !App.quiet then (printf "creating \"%s\"\n" config_path; flush stdout);
					if tpl = ""
					then Templater.generate_from_string dict config_template config_path
